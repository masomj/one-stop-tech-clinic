...
## One Stop Tech Clinic
  - This website is going to be the main online presence of the business "One Stop Tech Clinic". It will be replacing their facebook page as their primary focus. Therefore, it needs to be responsive and accessible across a range of devices, operating systems and screen sizes. It's goal is to bring in new customers for the computer repair business. So it needs to contain information about services that are offered, along with how to get them.
...
#### First time Visitor Goals
  ...
  2. As a first time visitor, I want to be able to easily and intuitively navigate the website
  3. As a first time visitor, I want to be able to see evidence of the business's work
  4. As a first time visitor, I want to be able to easily and intuitively contact the business in order to request a service
...
- #### Typography
   - As this site is aimed at providing information and attracting potential customers, an attractive and readable font called was Readex Pro chosen. A backup font of Sans Serif was set to reduce the possibility of an error resulting in the font defaulting to Times New Roman.
- #### Imagery
...
- Git and Github
  ...
  - I used Git via the command line in VS Code, though there is a GUI version available from GitHub, but as the industry standard is Command line, it is important to become proficient in Command line git before using the GUI version. 
### Testing
#### Responsive Testing
- Using Chrome's built in developer tools, which can be opened by right clicking on any webpage and pressing "Inspect", I tested the sites compatibility with different screen sizes. Chrome Developer tools has breakpoints for all standard screen widths for a variety of device types, like Tablets, Mobile Phones, 4K displays and Laptops. 
- In order to test the responsiveness, I viewed the site on multiple different devices, including the latest smartphones like the Google Pixel and Iphone X. The smallest screen width available to view through is 320px, so the site is optimised to 320px widths and above. 
- In order to ensure universal compatibility, I tested the website on a multitude of different browsers. These include: Microsoft Edge, Google Chrome, Firefox and Brave. 
...
[Lighthouse Test Report](Docs/Images/Lighthouse Report.png)
 ...
 - A custom 404 page was created, which includes the navigation bar, to ensure that if a user accidentally enters an incorrect address, they will be able to continue using the site.
 - Rather than using the Code Institute form dump, which can break the experience, upon completing the contact form, a custom thank you page is displayed. The form's data checks have been tested also, to ensure that the email form will validate an email addresses format. 
 ...
 - After the lighthouse testing, it was identified that my footers needed the list elements changed around. After doing this, they no were no longer centered. The cause was the change of order meant that they were no longer selected by the css rule. Targeting them as a child of the ul resolved it.
#### Code Validation
W3 Schools offer a code validation for both HTML and CSS. I ran all of my HTML pages and all CSS code through these. 
##### Results
After Running my code through the validator, several issues were identified. For instance, I was using an ID selector to target 2 elements, which should have been done via a class selector. This was a simple fix. 
...
##### First Time Visitor
  ...
  1. As a first time visitor, I want to be able to tell what services are offered soon after loading the website
  - This is achieved via the 4 information boxes on the home screen describing the services offered.
  2. As a first time visitor, I want to be able to easily and intuitively navigate the website
  - The Website has a clear navigation menu, with page titles that effectively communicate the content of that page.
  3. As a first time visitor, I want to be able to see evidence of the business's work.
...
- The social media icons in the footer were sourced from Font Awesome. 
- All Images were sourced from the "One Stop Tech Clinic Facebook Page", the commissioner’s of this site.
#### Acknowledgements
Slack forums for READme inspiration / Markdown guidance and advice on what GIT method to use. 

My mentor for advice on changing the colour scheme, and other great feedback. 
